...
public class RegistUtil {
	public static void sendEmail(JavaMailSender mailSender, String email, String id, String auth, int dice) {
		...
		String content = "%s님 맞으신가요?\n아래 링크를 통해 회원가입을 마무리할 수 있습니다!\n링크: http://ec2-3-34-77-222.ap-northeast-2.compute.amazonaws.com/member/emailcheck?id=%s&email=%s&auth=%s&dice=%d";// "������ȣ["+dice+"]";
		String rcontent = String.format(content, id, id, email, auth, dice);
		try {
