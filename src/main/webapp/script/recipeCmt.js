...
function modify_cmt(cmt_seq, cmt)  {
	...
	var mem_id = '2g1';	// 로그인 후 세션으로부터 받아야 한다!
	$.ajax({
		type: "POST",
...
// 게시글에 대한 댓글 작성하기
function writeCmtToRecipe(recipe_seq) {
	// 입력 검사
	...
	var mem_id = '2g1';	// 로그인 후 세션으로 받아야 한다!
...
function showRecipeCmtList(){
	$.ajax({
		...
		data: {recipe_seq: $("#get_recipe_seq").val()}
